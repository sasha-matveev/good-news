...
After the job succeeds, confirm the Python health endpoint before deploying the
new Python revision. Then perform the same API and scheduler smoke checks used
in rehearsal. Preserve the provider snapshot and backup through the rollback
window.

Application rollback is a Cloud Run revision change. Migrations are
forward-only and are not rolled back with application code.
