...
| `/internal/analysis/requests` | `backend/app/analysis_llm_service/main.py`, `backend/app/services/analysis_service_client.py`, `backend/app/services/analysis.py` | Retain as an explicit Java internal contract | This boundary is still the single explicit request/response contract between ingestion/batch orchestration and analysis execution in the legacy split runtime, and the repo keeps a dedicated client for it. |
...
| `backend/app/analysis_llm_service/` | Legacy analysis runtime still exists | Collapse into the new Java monolith while retaining an explicit internal analysis contract | The repo still preserves the analysis request boundary, but the active deployment topology is one backend service, not a separately deployed analysis service. |

This is the narrowest repo-grounded conclusion available today: the split runtimes are not the active production topology, so PR-01 should treat them as migration inputs to be collapsed into the Java monolith unless a boundary is explicitly preserved for internal coordination. The only boundary classified that way in this document is `/internal/analysis/requests`.
