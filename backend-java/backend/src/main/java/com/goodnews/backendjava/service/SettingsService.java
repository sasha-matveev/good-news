...
public class SettingsService {
    ...
    public Mono<AppSettings> loadSettings() {
        Mono<Map<String, String>> storedSettings = databaseClient
                .sql("SELECT key, value FROM settings")
                .map((row, metadata) ->
                        new StoredSettingRow(row.get("key", String.class), row.get("value", String.class)))
                .all()
                .collectMap(StoredSettingRow::key, StoredSettingRow::value, LinkedHashMap::new);

        Mono<Map<String, Boolean>> secretSettings = databaseClient
                .sql("SELECT key FROM secret_settings")
                .map((row, metadata) -> row.get("key", String.class))
                .all()
                .collectMap(key -> key, key -> true, LinkedHashMap::new);

        return Mono.zip(storedSettings, secretSettings).map(tuple -> {
            ...
            return new AppSettings(
                    ...
                    defaultIfBlank(
                            stored.get("analysis_summary_prompt"), DEFAULT_SETTINGS.get("analysis_summary_prompt")),
                    defaultIfBlank(
    ...
    public Mono<AppSettings> saveSettings(AppSettingsUpdate update) {
        Mono<Void> writes = Flux.concat(
                        ...
                        upsertSetting(
                                ...
                                Boolean.toString(update.dailyDigestEnabled()).toLowerCase()),
                        upsertSetting(
                                "daily_digest_catch_up_enabled",
                                Boolean.toString(update.dailyDigestCatchUpEnabled())
                                        .toLowerCase()),
                        upsertSetting(
                                "weekly_digest_enabled",
                                Boolean.toString(update.weeklyDigestEnabled()).toLowerCase()),
                        upsertSetting(
                                "weekly_digest_catch_up_enabled",
                                Boolean.toString(update.weeklyDigestCatchUpEnabled())
                                        .toLowerCase()),
                        upsertSetting("analysis_summary_prompt", blankToNull(update.analysisSummaryPrompt())),
                        upsertSetting(
                                "analysis_verdict_reason_prompt", blankToNull(update.analysisVerdictReasonPrompt())))
                .then();

        Mono<Void> secretWrite = hasText(update.smtpPassword())
                ? upsertSecretSetting(
                        "smtp_password", encryptSecret(update.smtpPassword(), appMasterKeyResolver.require()))
                : Mono.empty();
    ...
    public Mono<String> getSmtpPassword() {
        return databaseClient
                .sql("SELECT encrypted_value FROM secret_settings WHERE key = :key")
    ...
    private static byte[] deriveKeystream(String masterKey, byte[] nonce, int length) {
        ...
        while (keystream.position() < length) {
            byte[] block =
                    sha256(concat(seed, ByteBuffer.allocate(4).putInt(counter).array()));
    ...
    private static DatabaseClient.GenericExecuteSpec bindNullable(
            DatabaseClient.GenericExecuteSpec spec, String name, String value, Class<String> type) {
