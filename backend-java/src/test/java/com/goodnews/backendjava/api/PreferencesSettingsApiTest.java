...
class PreferencesSettingsApiTest {
    ...
    void getPreferencesReturnsPersistedAggregateExplanations() throws Exception {
        ...
        insertPost(1, 1, "https://alpha.example/posts/1", "Distributed tracing in practice", utc("2026-04-20T09:00:00Z"), "Deep incident review.");
        insertFeedback(1, "interesting");
        insertPost(2, 1, "https://alpha.example/posts/2", "Opinionated frontend note", utc("2026-04-21T09:00:00Z"), "Short take.");
        insertFeedback(2, "not_interesting");
        insertAnalysis(1, "Русское summary.", Map.of(
        ...
        insertAnalysis(2, "Short summary.", Map.of(
        ...
        assertThat(queryForString("SELECT summary FROM preference_profile WHERE id = 1"))
            .isEqualTo("From 2 feedback signals, the profile is learning toward distributed systems postmortems from Alpha and away from opinion pieces.");
    }

    @Test
    void recomputePreferencesCountsFeedbackWithoutAnalysisAndPersists() {
        insertSource(1, "Alpha");
        insertPost(1, 1, "https://alpha.example/posts/1", "Post without analysis", utc("2026-04-20T09:00:00Z"), "Content here.");
        insertFeedback(1, "interesting");
        ...
        webTestClient.post()
            ...
            .expectStatus().isOk()
            .expectBody()
            .jsonPath("$.feedback_totals.interesting").isEqualTo(1)
            .jsonPath("$.feedback_totals.total").isEqualTo(1);

        assertThat(queryForString("SELECT summary FROM preference_profile WHERE id = 1")).contains("Alpha");
    }
    ...
    void settingsApiReturnsDefaultsPersistsWriteOnlyPasswordAndResetsBlankPrompts() {
        webTestClient.get()
            ...
            .expectStatus().isOk()
            .expectBody()
            .jsonPath("$.daily_digest_time").isEqualTo("12:00")
            .jsonPath("$.weekly_digest_day_of_week").isEqualTo("sat")
            .jsonPath("$.weekly_digest_time").isEqualTo("23:30")
            .jsonPath("$.observability_dashboard_url")
            .isEqualTo("http://127.0.0.1:3000/d/good-news-overview/good-news-observability-overview")
            .jsonPath("$.smtp_port").isEqualTo(587)
            .jsonPath("$.smtp_security_mode").isEqualTo("starttls")
            .jsonPath("$.smtp_password_configured").isEqualTo(false)
        ...
        byte[] updateResponse = webTestClient.put()
            .uri("/api/settings")
            .bodyValue(settingsPayload().with("smtp_password", "plain-secret").build())
            .exchange()
            .expectStatus().isOk()
            .expectBody()
            .jsonPath("$.daily_digest_time").isEqualTo("08:45")
            .jsonPath("$.weekly_digest_day_of_week").isEqualTo("fri")
            .jsonPath("$.weekly_digest_time").isEqualTo("16:30")
            .jsonPath("$.smtp_password_configured").isEqualTo(true)
            .jsonPath("$.analysis_summary_prompt").isEqualTo(DEFAULT_SUMMARY_INSTRUCTIONS)
            .jsonPath("$.analysis_verdict_reason_prompt").isEqualTo(DEFAULT_VERDICT_REASON_INSTRUCTIONS)
            .returnResult()
            .getResponseBody();
        ...
        assertThat(queryForString("SELECT value FROM settings WHERE key = 'daily_digest_time'")).isEqualTo("08:45");
    ...
    void settingsApiRejectsInvalidUpdates() {
        webTestClient.put()
            ...
            .expectBody()
            .jsonPath("$.detail[?(@.loc[1]=='daily_digest_time')]").exists()
            .jsonPath("$.detail[?(@.loc[1]=='weekly_digest_day_of_week')]").exists()
            .jsonPath("$.detail[?(@.loc[1]=='weekly_digest_time')]").exists();
    }
    ...
    void settingsTestEmailUsesAdapterAndRunsOffEventLoop() {
        ...
        assertThat(recordingSmtpEmailAdapter.connectionSettings().securityMode()).isEqualTo("ssl");
