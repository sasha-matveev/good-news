...
class ReactiveSecurityIntegrationTest {
    ...
    void apiRequiresBearerTokenWhenFirebaseAuthConfigured() {
        webTestClient.get()
            ...
            .expectBody()
            .jsonPath("$.detail").isEqualTo("Missing bearer token.");
    }
    ...
    void apiRejectsNonAllowlistedEmail() {
        ...
        webTestClient.get()
            ...
            .expectBody()
            .jsonPath("$.detail").isEqualTo("Not allowed.");
    }

    @Test
    void apiAcceptsAllowlistedVerifiedEmail() {
        given(firebaseTokenVerifier.verify("owner"))
            .willReturn(Mono.just(new TokenClaims("owner@example.com", true)));
    ...
    void internalJobRunsForSchedulerServiceAccount() {
        ...
        webTestClient.post()
            ...
            .expectStatus().isOk()
            .expectBody()
            .jsonPath("$.processed_source_ids[0]").isEqualTo(1)
            .jsonPath("$.analyzed_pending").isEqualTo(true);
    }
