...
public class DigestHistoryService {
    ...
    public Flux<DigestDtos.DigestListItemResponse> listSentDigests() {
        return databaseClient
                ...
                .map((row, metadata) -> new DigestDtos.DigestListItemResponse(
                        ...
                        format(row.get("sent_at", OffsetDateTime.class)),
                        row.get("included_post_count", Long.class).intValue()))
                .all();
    }

    public Mono<DigestDtos.DigestDetailResponse> getSentDigest(long digestId) {
        Mono<DigestRow> digest = databaseClient
                .sql(
                        """
                ...
                .map((row, metadata) -> new DigestRow(
                        ...
                        row.get("html_body", String.class)))
                .one();
        return digest.flatMap(value -> includedPosts(digestId)
                        .collectList()
                        .map(posts -> new DigestDtos.DigestDetailResponse(
                                value.id(),
                                value.type(),
                                value.status(),
                                format(value.sentAt()),
    ...
    private Flux<DigestDtos.DigestIncludedPostResponse> includedPosts(long digestId) {
        return databaseClient
                ...
                """)
                .bind("id", digestId)
                .map((row, metadata) -> new DigestDtos.DigestIncludedPostResponse(
                        row.get("id", Long.class),
    ...
    private record DigestRow(
            long id, String type, String status, OffsetDateTime sentAt, String subject, String htmlBody) {}
}
