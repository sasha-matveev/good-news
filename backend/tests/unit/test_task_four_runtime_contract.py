...
def test_backend_runtime_runs_alembic_upgrade_before_uvicorn() -> None:
    ...
    startup_script = Path("backend/scripts/start-content-api-service.sh").read_text(encoding="utf-8")
    ...
    assert "backend/scripts/start-content-api-service.sh" in compose_text
    assert "python -m app.core.migration_runner" not in startup_script
    assert "pg_advisory_lock" in migration_runner
    assert "uvicorn app.content_api_service.main:app --host 0.0.0.0 --port" in startup_script
    assert "script_location = %(here)s/alembic" in alembic_ini
    assert "prepend_sys_path = %(here)s" in alembic_ini


def test_compose_uses_content_api_service_healthcheck_and_frontend_waits_for_it() -> None:
    compose_text = Path("docker-compose.yml").read_text(encoding="utf-8")
