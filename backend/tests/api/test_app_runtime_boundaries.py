...
def test_monolith_startup_owns_background_scheduler() -> None:
    engine = create_engine_from_url("sqlite+pysqlite:///:memory:")
    Base.metadata.create_all(engine)
    session_factory = create_session_factory(engine)
    stamp_schema_head(session_factory)
    app = create_monolith_app(session_factory=session_factory, enable_scheduler=True)
...
def test_monolith_startup_scheduler_can_be_disabled() -> None:
    engine = create_engine_from_url("sqlite+pysqlite:///:memory:")
    Base.metadata.create_all(engine)
    session_factory = create_session_factory(engine)
    stamp_schema_head(session_factory)
    app = create_monolith_app(session_factory=session_factory, enable_scheduler=False)
